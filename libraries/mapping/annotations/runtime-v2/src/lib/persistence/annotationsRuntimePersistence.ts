...
import {
  buildLinkedNodeGroupsFromLegacyNodes,
  reconcileLinkedNodeGroups,
...
export type AnnotationsRuntimePersistenceEnvelopeV5 = {
  ...
  tables: {
    annotationEntries: RuntimeAnnotationEntry[];
    nodes: RuntimeNode[];
    linkedNodeGroups: RuntimeLinkedNodeGroup[];
    edges: RuntimeEdge[];
  };
  settings: {
    elevationReferenceAnnotationId: string | null;
...
const cloneLinkedNodeGroup = (
  linkedNodeGroup: RuntimeLinkedNodeGroup
): RuntimeLinkedNodeGroup => ({
  ...linkedNodeGroup,
  nodeIds: [...linkedNodeGroup.nodeIds],
});
...
export const buildAnnotationsRuntimePersistenceState = (
  ...
  const filteredNodes = state.nodes
    ...
    .map(cloneNode);
  const filteredLinkedNodeGroups = reconcileLinkedNodeGroups({
    nodes: filteredNodes,
    linkedNodeGroups: state.linkedNodeGroups
      .map(cloneLinkedNodeGroup)
      .filter((linkedNodeGroup) =>
        linkedNodeGroup.nodeIds.some((nodeId) => usedNodeIds.has(nodeId))
      ),
  });
  ...
  return {
    ...
    tables: {
      annotationEntries,
      nodes: filteredNodes,
      linkedNodeGroups: filteredLinkedNodeGroups,
      edges: state.edges
...
export const resolvePersistedAnnotationsStoreState = ({
  ...
  const normalizedLinkedNodeGroups = reconcileLinkedNodeGroups({
    nodes: normalizedNodes,
    linkedNodeGroups:
      initialPersistenceState?.version === 5
        ? initialPersistenceState.tables.linkedNodeGroups.map(
            cloneLinkedNodeGroup
          )
        : buildLinkedNodeGroupsFromLegacyNodes(persistedLegacyNodes),
  });
  ...
  return {
    ...
    nodes: normalizedNodes,
    linkedNodeGroups: normalizedLinkedNodeGroups,
    ...
    if (parsed.version === 5) {
      ...
      return {
        ...
        tables: {
          ...
          linkedNodeGroups:
            parsed.tables.linkedNodeGroups.map(cloneLinkedNodeGroup),
