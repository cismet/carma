import { resolveRuntimePointLabelVisualDefaults } from "../config/runtime-point-label-visual-defaults";
import type {
  RuntimePointLabelRenderModel,
  RuntimePointMarkerRenderModel,
} from "./measurement-render-models";

const runtimeVisualizerInputDefaults = Object.freeze({
...
type BuildRuntimeVisualizerInputsArgs = BuildHostInteractionPointLabelsArgs &
...
const buildVisiblePointLabels = (
  ...
  pointLabels
    .map(resolveRuntimePointLabelVisualDefaults)
    .filter(
      (pointLabel) =>
        !(
  ...
  const normalizedPointLabels = !enableHostInteractionTargets
    ? visiblePointLabels.map((pointLabel) => ({
        ...
        longPressDurationMs:
          ...
          runtimeVisualizerInputDefaults.nodeLabelLongPressDurationMs,
      }))
    : visiblePointLabels.map((pointLabel) => {
...
export const buildRuntimeVisualizerInputs = ({
  points,
  ...args
}: BuildRuntimeVisualizerInputsArgs) => {
