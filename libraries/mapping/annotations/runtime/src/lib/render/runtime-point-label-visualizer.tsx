...
const runtimePointLabelVisualizerDefaults = Object.freeze({
  ...
  stemColor: runtimeMeasurementVisualDefaults.colors.surface,
  activeMoveGizmoLabelZIndex: RUNTIME_OVERLAY_DISTANCE_Z_INDEX.MAX + 1,
  typography: Object.freeze({
    defaultRootFontSizePx: 14,
  }),
});
...
const resolveCssFontSizePx = (
  fontSize: string = annotationTypographyDefaults.supportFontSizeRem
) => {
  ...
  if (!Number.isFinite(parsedFontSize)) {
    return (
      Number.parseFloat(annotationTypographyDefaults.supportFontSizeRem) *
      runtimePointLabelVisualizerDefaults.typography.defaultRootFontSizePx
    );
  }
...
const createEmptyLabelOverlayState = (): RuntimePointLabelOverlayState => ({
  ...
  hiddenByLayout: true,
  angleRad: 0,
  distance: runtimePointLabelVisualizerDefaults.layoutConfig.stemDistance,
  attach: "center",
  zIndex: 0,
});
    ...
    labelsRef.current.forEach((label, index) => {
      ...
      overlayZIndexById.set(
        ...
        isActiveMoveGizmoLabel
          ? runtimePointLabelVisualizerDefaults.activeMoveGizmoLabelZIndex
          : overlayZIndex
      );
    ...
    const layoutResult =
      layoutInputs.length > 0
        ? computePointLabelLayout({
            ...
            viewportHeight,
            cameraPitch,
            config: runtimePointLabelVisualizerDefaults.layoutConfig,
          })
        : EMPTY_LAYOUT_RESULT;
    ...
    labelsRef.current.forEach((label) => {
      ...
      nextStatesById.set(label.id, {
        ...
        distance:
          ...
          runtimePointLabelVisualizerDefaults.layoutConfig.stemDistance,
  ...
  useEffect(() => {
    ...
    normalizedLabels.forEach((label) => {
      ...
      const nextContentSignature = [
        ...
        `${label.fontFamily ?? ""}`,
        `${label.fontWeight ?? ""}`,
        `${label.longPressDurationMs ?? ""}`,
        `${label.markerOnlyPointerEvents ?? false}`,
        getOverlayReferenceSignature(label.content),
        getOverlayReferenceSignature(effectiveBadgeContent),
        getOverlayReferenceSignature(clickBlocked ? undefined : label.onClick),
        getOverlayReferenceSignature(
          blockLabelInteractions ? undefined : label.onDoubleClick
        ),
        getOverlayReferenceSignature(label.onHoverChange),
        getOverlayReferenceSignature(
      ...
      const overlayElementUpdate: Partial<LabelOverlayElement> = {
        ...
        updatePosition: (elementDiv: HTMLElement) => {
          ...
          const markerOuterRadius = label.hideMarker
            ? 0
            : (label.markerPixelSize ??
                runtimePointLabelVisualizerDefaults.markerPixelSize) /
                2 +
              (label.markerOutlineWidth ??
                runtimePointLabelVisualizerDefaults.markerOutlineWidth) /
                2;
          ...
          stemLine.style.borderBottom = `1px ${
            ...
            label.lineColor ?? runtimePointLabelVisualizerDefaults.stemColor
          }`;
