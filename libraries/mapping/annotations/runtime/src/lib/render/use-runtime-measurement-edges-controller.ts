...
const runtimeMeasurementEdgeDefaults = Object.freeze({
  dashPattern: Object.freeze({
    dashLengthMeters: 1.5,
    gapLengthMeters: 1.5,
    dashLengthPx: 6,
    gapLengthPx: 8,
  }),
  ...
  distanceTriangleCorner: Object.freeze({
    dotRadiusPx: distanceToolVisualDefaults.cornerOverlay.strokeWidthPx / 2,
  }),
});
...
const buildLineSegments = (
  ...
  if (totalLength <= runtimeMeasurementEdgeDefaults.minSegmentLengthMeters) {
    return [];
  }
  ...
  const safeDashLength = Math.max(
    dashLength,
    runtimeMeasurementEdgeDefaults.minSegmentLengthMeters
  );
  const safeGapLength = Math.max(gapLength, 0);
  const safeCapLength = Math.min(
    Math.max(capLength, 0),
    totalLength * 0.5 -
      runtimeMeasurementEdgeDefaults.minSegmentLengthMeters * 0.5
  );
  const step = Math.max(
  ...
  if (safeCapLength <= runtimeMeasurementEdgeDefaults.minSegmentLengthMeters) {
  ...
  if (
    totalLength <=
    safeCapLength * 2 + runtimeMeasurementEdgeDefaults.minSegmentLengthMeters
  ) {
    return [[start, end]];
  }
...
const createSceneLineHandle = (
  ...
  const dashLengthMeters = line.dashed
    ? Math.max(
        runtimeMeasurementEdgeDefaults.dashPattern.dashLengthPx *
          metersPerPixel,
        runtimeMeasurementEdgeDefaults.minSegmentLengthMeters
      )
    : runtimeMeasurementEdgeDefaults.dashPattern.dashLengthMeters;
  const gapLengthMeters = line.dashed
    ? Math.max(
        runtimeMeasurementEdgeDefaults.dashPattern.gapLengthPx * metersPerPixel,
        0
      )
    : runtimeMeasurementEdgeDefaults.dashPattern.gapLengthMeters;
  const capLengthMeters = line.dashed
    ? Math.max(
        line.strokeWidth * metersPerPixel,
        runtimeMeasurementEdgeDefaults.minSegmentLengthMeters * 2
      )
    : 0;
...
const createDistanceTriangleCornerHandle = (
  ...
  const svg = document.createElementNS(
    runtimeMeasurementEdgeDefaults.svgNamespace,
    "svg"
  );
  ...
  dot.setAttribute(
    "r",
    `${runtimeMeasurementEdgeDefaults.distanceTriangleCorner.dotRadiusPx}`
  );
    ...
    const updateDistanceTriangleLabels = () => {
      distanceTriangleEdges.forEach((edge) => {
        ...
        if (
          ...
          const handleCornerClick = onDistanceTriangleCornerClick
            ? () =>
                onDistanceTriangleCornerClick(
