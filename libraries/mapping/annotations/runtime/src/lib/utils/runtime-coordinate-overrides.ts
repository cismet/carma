...
import { areRuntimeCoordinatesEqual } from "./runtime-coordinate-equality";
import type {
...
export type RuntimeNodeCoordinateOverrides = Readonly<
  Record<string, RuntimeCoordinate>
>;

export const EMPTY_RUNTIME_NODE_COORDINATE_OVERRIDES =
  {} as RuntimeNodeCoordinateOverrides;

export const hasRuntimeCoordinateOverrides = (
  coordinateOverrides: RuntimeNodeCoordinateOverrides
) => Object.keys(coordinateOverrides).length > 0;

export const areRuntimeCoordinateOverridesEqual = (
  left: RuntimeNodeCoordinateOverrides,
  right: RuntimeNodeCoordinateOverrides
) => {
  ...
  return leftKeys.every((key) => {
    ...
    return areRuntimeCoordinatesEqual(leftCoordinate, rightCoordinate);
  });
};

export const applyRuntimeCoordinateOverridesToNodes = (
  nodes: readonly RuntimeNode[],
  coordinateOverrides: RuntimeNodeCoordinateOverrides
) => {
  if (!hasRuntimeCoordinateOverrides(coordinateOverrides)) {
    return nodes;
  }
...
export const applyRuntimeCoordinateOverridesToVisualModels = (
