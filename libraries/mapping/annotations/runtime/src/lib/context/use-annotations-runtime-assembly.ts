...
} from "../utils/runtime-annotation-coordinates";
import { createAnnotationToolDraftStore } from "../interaction/lifecycle/create-annotation-tool-draft-store";
import { flyToAnnotationPoints } from "./runtime-annotation-fly-to";
import {
...
export const useAnnotationsRuntimeAssembly = ({
  scene,
  ...
  const [activeMoveGizmoNodeId, setActiveMoveGizmoNodeId] = useState<
    string | null
  >(null);
  const [hoveredPointQueryNodeId, setHoveredPointQueryNodeId] = useState<
    string | null
  >(null);
  const [cursorOverlayVisible, setCursorOverlayVisible] = useState(false);
  const setCursorOverlayEnabled = useCallback((enabled: boolean) => {
    setCursorOverlayVisible(enabled);
  }, []);
  ...
  const services = useMemo(
    () => ({
      ...
      addAnnotation,
      setActiveToolType,
      requestModeChange: (toolType: RuntimeToolId) =>
        lifecycleHostApiRef.current.requestModeChange(toolType),
      requestStartMeasurement: (toolType?: RuntimeToolId) =>
