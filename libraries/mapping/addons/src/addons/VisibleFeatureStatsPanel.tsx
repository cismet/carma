...
/**
 * Readout for what `VisibleFeatureStatsSource` publishes on the
 * `visibleFeatureStats` channel: a proportion bar and a per-category list of
 * what is on screen, registered into the host's control layout.
 *
 * Map-agnostic — it reads one channel and picks the colours, so a route can
 * replace it with a different consumer without touching the query side.
 *
 * Without the source addon the channel stays empty and the panel reads as
 * loading; `AddonHost` warns about the missing pairing in dev, off the
 * `provides`/`requires` declarations in the registry.
 */
 ...
 * reads the same in a story, in BELIS, or in a host with a different body font.
 */
const FONT_STACK =
  '-apple-system, BlinkMacSystemFont, "Segoe UI", "Roboto", "Oxygen", "Ubuntu", "Cantarell", "Fira Sans", "Droid Sans", "Helvetica Neue", sans-serif';

/**
 * Categorical slots validated against a white surface: adjacent pairs keep
 * ΔE ≥ 8 (OKLab ×100) under protanopia, deuteranopia and tritanopia. Three slots
 * stay below 3:1 contrast, so every swatch is paired with a label and a number —
 * colour never carries a value alone.
 *
 * Assigned per group key in first-seen order (`useStableSeriesColors`), never by
 * rank: rows are sorted by count, and panning reorders them.
 */
const SERIES_COLORS = [
  "#2a78d6", // blue
  "#eb6834", // orange
...
/**
 * Legend mark. The shape repeats the geometry the group is drawn with, as a
 * second, non-colour channel for identity.
 */
...
/**
 * Presentational half: props in, markup out, no map and no channel, so it can be
 * rendered from a story or a test without a MapLibre instance.
 *
 * Stateless by design — `Control` re-registers its children on every render, so
 * local state here would be reset on every pan.
 */
export const StatsReadout = ({
  ...
  // built from the rows rather than from `totalCount`, so the segments add up to
  ...
  // the settled viewport stays on screen while the next one is queried, dimmed
  // rather than replaced by a skeleton
  ...
  return (
    ...
    >
      <div className="flex items-baseline justify-between gap-3">
        {/* the filtered heading is the longer of the two, so it shrinks rather
            than wrapping the header */}
        <span
        ...
        >
          ...
          {/* the denominator says what the number is a share of, so a small
          ...
          {segments.length === 0 ? (
            ...
            segments.map((segment) => (
              <span
                ...
                // grow by count, but never thinner than a visible sliver
                style={{
...
/**
 * Group key -> palette slot, in first-seen order and kept for the life of the
 * addon. Rows are sorted by count and panning reorders them, so indexing the
 * palette by row position would repaint groups on every pan. Slots run out after
 * eight; later groups share the neutral of the folded tail.
 */
...
/** nothing published yet — the source is mounted, its first query is not done */
...
export const VisibleFeatureStatsPanel = ({
  ...
  // registers the panel into the surrounding `ControlLayout`, which draws it in
  // that corner; nothing is rendered here
  return (
    <Control position={position} order={order}>
      <StatsReadout
        totalCount={stats?.totalCount ?? 0}
        groups={coloredGroups}
        // before the first publish the query is genuinely still running
