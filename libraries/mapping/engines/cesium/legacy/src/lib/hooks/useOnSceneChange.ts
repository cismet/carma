...
export const useOnSceneChange = (
  ...
  isSecondaryStyle: boolean = false
) => {
  const ctx = useCesiumContext();
  const { getScene, sceneAnimationMapRef, isAnimating } = ctx;
  ...
  useEffect(() => {
    ...
    if (scene && sceneAnimationMapRef.current?.has(scene)) {
      return; // Skip if animating
    }
  ...
  }, [ctx, isCesiumActive, isSecondaryStyle, getScene, sceneAnimationMapRef]);
  ...
  }, [ctx, isSecondaryStyle, isCesiumActive, onSceneChange, isAnimating]);
};
