...
describe("cesium codec zoom convention", () => {
  it("applies +1 offset when using LEAFLET_256", () => {
    const codec = createCesiumViewStateHashCodec({
      zoomConvention: HASH_ZOOM_CONVENTION.LEAFLET_256,
    });
    const params = codec.encode(make2dViewState());
  ...
  it("round-trips zoom through +1/-1 offset", () => {
    ...
    const original = make2dViewState({ zoom: 14.5 });
...
describe("cesium codec FOV encode", () => {
  it("encodes longer-edge fov from fovLongerEdge", () => {
    const codec = createCesiumViewStateHashCodec({ defaultFovDeg: 60 });
...
describe("cesium codec FOV decode", () => {
...
describe("cesium codec round-trips", () => {
  const codec = createCesiumViewStateHashCodec({
    zoomConvention: HASH_ZOOM_CONVENTION.LEAFLET_256,
    defaultFovDeg: 60,
  });
  ...
  it("round-trips a view state with non-zero roll", () => {
    const original = make3dViewState({ roll: asRadians(15) });
    const encoded = codec.encode(original);
...
describe("geoportal-realistic round-trips", () => {
  const geoportalCodec = createCesiumViewStateHashCodec({
    zoomConvention: HASH_ZOOM_CONVENTION.LEAFLET_256,
    defaultFovDeg: 60,
  });
