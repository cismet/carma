...
const sliceCesium = createSlice({
  ...
  reducers: {
    ...
    setTransitionTo2d: (state: CesiumState) => {
      ...
      state.currentTransition = CESIUM_RUNTIME_TRANSITION_STATE.TO2D;
    },
    setTransitionTo3d: (state: CesiumState) => {
      console.debug("REDUCER [STATE|CESIUM] transition to 3D");
      state.currentTransition = CESIUM_RUNTIME_TRANSITION_STATE.TO3D;
    },
    clearTransition: (state: CesiumState) => {
      console.debug("REDUCER [STATE|CESIUM] transition cleared");
      state.currentTransition = CESIUM_RUNTIME_TRANSITION_STATE.NONE;
    },
...
export const selectCesiumRuntimeIsAnimating = (state: RootState | undefined) =>
  selectCesiumState(state)?.isAnimating ?? initialState.isAnimating;
export const selectCesiumRuntimeCurrentTransition = (state: RootState | undefined) =>
  selectCesiumState(state)?.currentTransition ?? initialState.currentTransition;
export const selectCesiumRuntimeIsTransitioning = (state: RootState | undefined) =>
  selectCesiumRuntimeCurrentTransition(state) !== CESIUM_RUNTIME_TRANSITION_STATE.NONE;

export const selectCesiumRuntimeModels = (state: RootState | undefined) =>
