...
const hideLayers = (ctx: CesiumContextType) => {
  ctx.withScene((scene) => {
    const hideOnce = () => {
      ctx.withScene((scene, runtime) => {
        for (let i = 0; i < runtime.imageryLayers.length; i++) {
          const layer = runtime.imageryLayers.get(i);
          if (isValidImageryLayer(layer)) {
            layer.show = false; // Hide the layer
          } else {
            console.debug("[CESIUM|RUNTIME] skip invalid imagery layer");
          }
        }
