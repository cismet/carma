...
// TODO MOVE THE ID into runtime config/state
...
const waitAndSetTerrainProvider = (
  ...
  const checkTerrainProvider = () => {
    ...
    const hasProvider = ctx.withTerrainProvider((terrainProvider, runtime) => {
      console.debug(
  ...
  const isSet = ctx.withTerrainProvider((terrainProvider, runtime) => {
    runtime.scene.terrainProvider = terrainProvider;
    isTerrainProviderSet = true;
    onReady?.();
