...
export const buildDistanceRelationRenderContext = ({
  ...
  const polygonEdgeRelationIds =
    getSplitMarkerRelationIdSet(planarPolygonGroups);
  const planarPolygonSharedEdgeRelationIds =
    getPlanarPolygonSharedEdgeRelationIdSet(planarPolygonGroups);
  ...
  const midpointTickRelationIds = getSplitMarkerRelationIdSetForGroups(
    planarPolygonGroups,
  ...
  const selectedOrActiveOpenPolylineRelationIds = new Set<string>();
  planarPolygonGroups.forEach((group) => {
    ...
    (group.edgeRelationIds ?? []).forEach((relationId) => {
      selectedOrActiveOpenPolylineRelationIds.add(relationId);
    });
  });
