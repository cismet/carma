...
import {
  ...
  useMeasurementLivePreviewState,
  useCesiumOverlaySync,
  type CesiumLabelLayoutConfigOverrides,
  type MeasurementLivePreviewDescriptor,
...
import {
  ...
  type SurfaceType,
  type MeasurementMode as MeasurementModeType,
} from "../types/MeasurementTypes";
import type {
  MeasurementScriptAddLabelParams,
  MeasurementScriptAddMeasurementParams,
  MeasurementScriptAddNodeParams,
  MeasurementScriptApiOptions,
  MeasurementScriptRemoveMeasurementParams,
  MeasurementScriptRpcId,
  MeasurementScriptRpcRequest,
  MeasurementScriptRpcResponse,
  MeasurementScriptStatePatch,
  MeasurementScriptStatePatchStrategy,
  MeasurementScriptStateSnapshot,
  MeasurementScriptUpdateMeasurementParams,
  MeasurementScriptWindowApi,
} from "../types/MeasurementScriptApi";
...
type MeasurementMode = MeasurementModeType;
const CESIUM_MEASUREMENT_MODE = {
  NONE: "none",
  ...
  Traverse: "traverse",
  Elevation: "elevation",
} as const satisfies Record<string, MeasurementMode>;
...
export type MeasurementProviderOptions = {
  ...
  scriptApi?: MeasurementScriptApiOptions;
};

const defaultOptions: MeasurementProviderOptions = {
  temporary: false,
  mode: CESIUM_MEASUREMENT_MODE.PointMeasure,
...
const DEFAULT_MEASUREMENT_SCRIPT_API_NAMESPACE = "CARMA_MEASUREMENTS_SCRIPT";
const DEFAULT_MEASUREMENT_SCRIPT_API_REQUEST_EVENT =
  "carma:measurements:script:request";
const DEFAULT_MEASUREMENT_SCRIPT_API_RESPONSE_EVENT =
  "carma:measurements:script:response";
const MEASUREMENT_MODE_VALUE_SET = new Set(
  Object.values(CESIUM_MEASUREMENT_MODE)
);
...
const coerceCartesian3 = (value: unknown): Cartesian3 | null => {
  if (value instanceof Cartesian3) {
    return Cartesian3.clone(value, new Cartesian3());
  }
...
const cloneMeasurementGeometryECEFForScriptState = (
  geometry: Cartesian3 | Cartesian3[]
): Cartesian3 | Cartesian3[] =>
  Array.isArray(geometry)
    ? geometry.map((point) => Cartesian3.clone(point, new Cartesian3()))
    : Cartesian3.clone(geometry, new Cartesian3());

const cloneMeasurementCollectionForScriptState = (
  entries: MeasurementCollection
): MeasurementCollection =>
  entries.map((entry) => ({
    ...entry,
    geometryECEF: cloneMeasurementGeometryECEFForScriptState(
      entry.geometryECEF
    ),
  }));

const cloneDistanceRelationsForScriptState = (
  relations: PointDistanceRelation[]
): PointDistanceRelation[] =>
  relations.map((relation) => ({
    ...relation,
    labelVisibilityByKind: relation.labelVisibilityByKind
      ? { ...relation.labelVisibilityByKind }
      : undefined,
  }));

const clonePlanarPolygonGroupsForScriptState = (
  groups: PlanarPolygonGroup[]
): PlanarPolygonGroup[] =>
  groups.map((group) => ({
    ...group,
    vertexPointIds: [...group.vertexPointIds],
    edgeRelationIds: [...group.edgeRelationIds],
    plane: group.plane
      ? {
          anchorECEF: { ...group.plane.anchorECEF },
          normalECEF: { ...group.plane.normalECEF },
        }
      : undefined,
    planarPolygonLocalFrame: group.planarPolygonLocalFrame
      ? {
          originECEF: { ...group.planarPolygonLocalFrame.originECEF },
          eastECEF: { ...group.planarPolygonLocalFrame.eastECEF },
          northECEF: { ...group.planarPolygonLocalFrame.northECEF },
          upECEF: { ...group.planarPolygonLocalFrame.upECEF },
        }
      : undefined,
  }));
...
> = ({ children, options }) => {
  ...
  const scriptApiOptions = normalizeOptions<MeasurementScriptApiOptions>(
    options?.scriptApi,
    {
      enabled: true,
      namespace: DEFAULT_MEASUREMENT_SCRIPT_API_NAMESPACE,
      requestEventName: DEFAULT_MEASUREMENT_SCRIPT_API_REQUEST_EVENT,
      responseEventName: DEFAULT_MEASUREMENT_SCRIPT_API_RESPONSE_EVENT,
    }
  );
  const {
  ...
  const [defaultPolylineSegmentLineMode, setDefaultPolylineSegmentLineMode] =
    useState<PolylineSegmentLineMode>(DEFAULT_LINEAR_SEGMENT_LINE_MODE);
  ...
  const activeLivePreviewDescriptor =
    useMemo<MeasurementLivePreviewDescriptor>(() => {
      ...
      if (measurementMode === CESIUM_MEASUREMENT_MODE.PointQuery) {
        return {
          type: "distance",
          verticalOffsetMeters: 0,
        };
      ...
      if (measurementMode !== CESIUM_MEASUREMENT_MODE.PolylineMeasure) {
        return {
          type: "none",
          verticalOffsetMeters: 0,
        };
  ...
  const showPoints = !hideMeasurementsOfType.has(
    CESIUM_MEASUREMENT_MODE.PointQuery
  );
  ...
  const showPointLabels =
    showPoints &&
    showLabels &&
    !hideLabelsOfType.has(CESIUM_MEASUREMENT_MODE.PointQuery);
        ...
        setMeasurements((prev) => {
          ...
          const autoCornerEntries = createdFacadeAutoCorners.map(
            ({ id, position }, index) => {
              ...
              return {
                type: CESIUM_MEASUREMENT_MODE.PointQuery,
                id,
  ...
  >(
    () => ({
      [CESIUM_MEASUREMENT_MODE.PointMeasure]: (id) =>
        handlePointMeasurePointCreated(id),
      [CESIUM_MEASUREMENT_MODE.PointQuery]: (id, positionECEF) =>
        handleDistancePointCreated(id, positionECEF),
      [CESIUM_MEASUREMENT_MODE.PolylineMeasure]: (id, positionECEF) =>
  ...
  useEffect(() => {
    if (measurementMode === CESIUM_MEASUREMENT_MODE.PolylineMeasure) return;
    setPendingPolylinePromotionRingClosurePointId(null);
  }, [measurementMode]);
  ...
  const clearMeasurementsByType = useCallback((type: MeasurementMode) => {
    ...
    if (type === CESIUM_MEASUREMENT_MODE.PointQuery) {
      setDistanceRelations([]);
      setPlanarPolygonGroups([]);
      setSelectedPlanarPolygonGroupId(null);
      setActivePlanarPolygonGroupId(null);
  ...
  useEffect(() => {
    if (options?.mode !== undefined) {
      ...
      if (options.mode === CESIUM_MEASUREMENT_MODE.NONE) {
        setSelectedMeasurementId(null);
        setPreviousSelectedMeasurementId(null);
        setDoubleClickChainSourcePointId(null);
        setSelectedPlanarPolygonGroupId(null);
  ...
  useEffect(() => {
    if (mapMeasurements.mode === MAP_MEASUREMENT_MODE.MEASUREMENT) {
      setMeasurementMode((prev) =>
        prev === CESIUM_MEASUREMENT_MODE.NONE
          ? CESIUM_MEASUREMENT_MODE.PointMeasure
          : prev
      );
    } else {
      setMeasurementMode(CESIUM_MEASUREMENT_MODE.NONE);
      setSelectedMeasurementId(null);
      setPreviousSelectedMeasurementId(null);
      setDoubleClickChainSourcePointId(null);
      setSelectedPlanarPolygonGroupId(null);
  ...
  useEffect(() => {
    ...
    if (
      previousMode === CESIUM_MEASUREMENT_MODE.PointQuery &&
      measurementMode !== CESIUM_MEASUREMENT_MODE.PointQuery &&
      doubleClickChainSourcePointId
    ) {
      ...
      if (
        ...
        isPointMeasurementEntry(pendingSourceMeasurement) &&
  ...
  const scriptMeasurementModeRef = useRef(measurementMode);
  const scriptMeasurementsRef = useRef(measurements);
  const scriptDistanceRelationsRef = useRef(distanceRelations);
  const scriptPlanarPolygonGroupsRef = useRef(planarPolygonGroups);
  const scriptSelectedMeasurementIdRef = useRef(selectedMeasurementId);
  const scriptSelectedMeasurementIdsRef = useRef(selectedMeasurementIds);
  const scriptSelectedPlanarPolygonGroupIdRef = useRef(
    selectedPlanarPolygonGroupId
  );

  useEffect(() => {
    scriptMeasurementModeRef.current = measurementMode;
  }, [measurementMode]);
  useEffect(() => {
    scriptMeasurementsRef.current = measurements;
  }, [measurements]);
  useEffect(() => {
    scriptDistanceRelationsRef.current = distanceRelations;
  }, [distanceRelations]);
  useEffect(() => {
    scriptPlanarPolygonGroupsRef.current = planarPolygonGroups;
  }, [planarPolygonGroups]);
  useEffect(() => {
    scriptSelectedMeasurementIdRef.current = selectedMeasurementId;
  }, [selectedMeasurementId]);
  useEffect(() => {
    scriptSelectedMeasurementIdsRef.current = selectedMeasurementIds;
  }, [selectedMeasurementIds]);
  useEffect(() => {
    scriptSelectedPlanarPolygonGroupIdRef.current =
      selectedPlanarPolygonGroupId;
  }, [selectedPlanarPolygonGroupId]);

  const getScriptStateSnapshot = useCallback(
    (): MeasurementScriptStateSnapshot => ({
      measurementMode: scriptMeasurementModeRef.current,
      selectedMeasurementId: scriptSelectedMeasurementIdRef.current,
      selectedMeasurementIds: [...scriptSelectedMeasurementIdsRef.current],
      selectedPlanarPolygonGroupId:
        scriptSelectedPlanarPolygonGroupIdRef.current,
      measurements: cloneMeasurementCollectionForScriptState(
        scriptMeasurementsRef.current
      ),
      distanceRelations: cloneDistanceRelationsForScriptState(
        scriptDistanceRelationsRef.current
      ),
      planarPolygonGroups: clonePlanarPolygonGroupsForScriptState(
        scriptPlanarPolygonGroupsRef.current
      ),
    }),
    []
  );

  const setScriptState = useCallback(
    (
      nextMeasurements: MeasurementCollection,
      nextDistanceRelations: PointDistanceRelation[],
      nextPlanarPolygonGroups: PlanarPolygonGroup[]
    ) => {
      scriptMeasurementsRef.current = nextMeasurements;
      scriptDistanceRelationsRef.current = nextDistanceRelations;
      scriptPlanarPolygonGroupsRef.current = nextPlanarPolygonGroups;
      setMeasurements(nextMeasurements);
      setDistanceRelations(nextDistanceRelations);
      setPlanarPolygonGroups(nextPlanarPolygonGroups);
    },
    [setDistanceRelations, setMeasurements, setPlanarPolygonGroups]
  );

  const coerceScriptMeasurementEntry = useCallback(
    (entry: MeasurementEntry): MeasurementEntry | null => {
      if (!entry?.id || !entry.type) return null;

      if (entry.type === CESIUM_MEASUREMENT_MODE.PointQuery) {
        const geometryFromEntry = coerceCartesian3(entry.geometryECEF);
        let geometryECEF = geometryFromEntry;
        if (!geometryECEF && isObjectRecord(entry.geometryWGS84)) {
          const longitude = Number(entry.geometryWGS84.longitude);
          const latitude = Number(entry.geometryWGS84.latitude);
          const height = Number(entry.geometryWGS84.height ?? 0);
          if (Number.isFinite(longitude) && Number.isFinite(latitude)) {
            geometryECEF = Cartesian3.fromDegrees(
              longitude,
              latitude,
        ...
        if (!geometryECEF) return null;

        let longitude = Number.NaN;
        let latitude = Number.NaN;
        let height = Number.NaN;
        if (isObjectRecord(entry.geometryWGS84)) {
          longitude = Number(entry.geometryWGS84.longitude);
          latitude = Number(entry.geometryWGS84.latitude);
          height = Number(entry.geometryWGS84.height);
        }
        if (
          ...
          const derived = getDegreesFromCartesian(geometryECEF);
          longitude = derived.longitude;
          latitude = derived.latitude;
          height = derived.altitude ?? 0;
        }
        ...
        return {
          ...
          pointLabelMode:
            entry.pointLabelMode ?? DEFAULT_POINT_LABEL_METRIC_MODE,
          labelAnchor: normalizeMeasurementLabelAnchor(entry.labelAnchor),
          labelAppearance: normalizeMeasurementLabelAppearance(
            entry.labelAppearance
          ),
          geometryECEF,
          geometryWGS84: {
            longitude,
      ...
      if (entry.type === CESIUM_MEASUREMENT_MODE.Traverse) {
        if (!Array.isArray(entry.geometryECEF)) return null;
        const geometryECEF = entry.geometryECEF
          .map((point) => coerceCartesian3(point))
          .filter((point): point is Cartesian3 => Boolean(point));
        if (geometryECEF.length === 0) return null;

        const geometryWGS84 =
          Array.isArray(entry.geometryWGS84) &&
          entry.geometryWGS84.length === geometryECEF.length
            ? entry.geometryWGS84.map((point, index) => {
                ...
                const derived = getDegreesFromCartesian(geometryECEF[index]);
                return {
                  longitude: derived.longitude,
                  latitude: derived.latitude,
                  height: derived.altitude ?? 0,
            ...
            : geometryECEF.map((point) => {
                const derived = getDegreesFromCartesian(point);
                return {
                  longitude: derived.longitude,
                  latitude: derived.latitude,
                  height: derived.altitude ?? 0,
        ...
        return {
          ...entry,
          id: entry.id.trim(),
          type: CESIUM_MEASUREMENT_MODE.Traverse,
          geometryECEF,
          geometryWGS84,
        };
      }
  ...
  const coerceScriptDistanceRelation = useCallback(
    (relation: PointDistanceRelation): PointDistanceRelation | null => {
      const id = relation.id?.trim();
      const pointAId = relation.pointAId?.trim();
      const pointBId = relation.pointBId?.trim();
      if (!id || !pointAId || !pointBId) return null;
      return withDistanceRelationEdgeId({
        ...relation,
        id,
        pointAId,
        pointBId,
        anchorPointId: relation.anchorPointId?.trim() || pointAId,
      });
    },
    []
  );

  const coerceScriptPlanarPolygonGroup = useCallback(
    (
      group: PlanarPolygonGroup,
      pointById: Map<string, Cartesian3>
      ...
      const nextVertexPointIds = group.vertexPointIds.filter((pointId) =>
        pointById.has(pointId)
      );
      if (nextVertexPointIds.length === 0) return null;
      const nextClosed = group.closed && nextVertexPointIds.length >= 3;
      const nextEdgeRelationIds = buildEdgeRelationIdsForPolygon(
        nextVertexPointIds,
        nextClosed,
      ...
      return computePolygonGroupDerivedData(
        {
          ...
          id,
          closed: nextClosed,
          measurementKind: getPlanarGroupMeasurementKind(group),
          vertexPointIds: nextVertexPointIds,
          edgeRelationIds: nextEdgeRelationIds,
        },
        pointById
  ...
  const applyScriptStatePatch = useCallback(
    (patch: MeasurementScriptStatePatch) => {
      const strategy: MeasurementScriptStatePatchStrategy =
        patch.strategy === "replace" ? "replace" : "merge";

      const incomingMeasurements = (patch.measurements ?? [])
        .map((entry) => coerceScriptMeasurementEntry(entry))
        .filter((entry): entry is MeasurementEntry => Boolean(entry));
      const nextMeasurements =
        patch.measurements === undefined
          ? scriptMeasurementsRef.current
          : strategy === "replace"
          ? incomingMeasurements
          : mergeById(scriptMeasurementsRef.current, incomingMeasurements);

      const pointById = getPointPositionMap(nextMeasurements);
      const pointIdSet = new Set(Array.from(pointById.keys()));

      const incomingDistanceRelations = (patch.distanceRelations ?? [])
        .map((relation) => coerceScriptDistanceRelation(relation))
        .filter((relation): relation is PointDistanceRelation =>
          Boolean(relation)
        )
        .filter(
          (relation) =>
            pointIdSet.has(relation.pointAId) &&
            pointIdSet.has(relation.pointBId)
        );
      const nextDistanceRelations =
        patch.distanceRelations === undefined
          ? scriptDistanceRelationsRef.current
          : strategy === "replace"
          ? incomingDistanceRelations
          : mergeById(
              scriptDistanceRelationsRef.current,
              incomingDistanceRelations
            ).filter(
      ...
      const incomingPlanarPolygonGroups = (patch.planarPolygonGroups ?? [])
        .map((group) => coerceScriptPlanarPolygonGroup(group, pointById))
        .filter((group): group is PlanarPolygonGroup => Boolean(group));
      const nextPlanarPolygonGroups =
        patch.planarPolygonGroups === undefined
          ? scriptPlanarPolygonGroupsRef.current
          : strategy === "replace"
          ? incomingPlanarPolygonGroups
          : mergeById(
              scriptPlanarPolygonGroupsRef.current,
              incomingPlanarPolygonGroups
            );
      ...
      if (patch.selection) {
        const {
          ...
          measurementIds,
          additive,
          planarPolygonGroupId,
        } = patch.selection;
        if (Array.isArray(measurementIds)) {
          selectMeasurementIds(measurementIds, additive);
        } else if (measurementId !== undefined) {
          selectMeasurementById(measurementId ?? null);
        }
        if (planarPolygonGroupId !== undefined) {
          selectPlanarPolygonGroupById(planarPolygonGroupId ?? null);
        }
      }

      return {
        measurements: nextMeasurements.length,
        distanceRelations: nextDistanceRelations.length,
        planarPolygonGroups: nextPlanarPolygonGroups.length,
      };
    },
    [
      coerceScriptDistanceRelation,
      coerceScriptMeasurementEntry,
      coerceScriptPlanarPolygonGroup,
      selectMeasurementById,
      selectMeasurementIds,
      selectPlanarPolygonGroupById,
      setScriptState,
  ...
  const upsertScriptMeasurementEntry = useCallback(
    (
      nextEntry: MeasurementEntry,
      options?: { upsertIfExists?: boolean; select?: boolean }
    ) => {
      const existingMeasurement = scriptMeasurementsRef.current.find(
        (measurement) => measurement.id === nextEntry.id
      );
      if (existingMeasurement && !options?.upsertIfExists) {
        throw new Error(
          `measurement id '${nextEntry.id}' already exists (set upsertIfExists=true to replace)`
        );
      }

      const nextMeasurements = existingMeasurement
        ? scriptMeasurementsRef.current.map((measurement) =>
            measurement.id === nextEntry.id ? nextEntry : measurement
          )
        : [...scriptMeasurementsRef.current, nextEntry];
      const nextPointById = getPointPositionMap(nextMeasurements);
      const nextDistanceRelations = scriptDistanceRelationsRef.current.filter(
        (relation) =>
          nextPointById.has(relation.pointAId) &&
          nextPointById.has(relation.pointBId)
      );
      const nextPlanarPolygonGroups = scriptPlanarPolygonGroupsRef.current
        .map((group) => coerceScriptPlanarPolygonGroup(group, nextPointById))
        .filter((group): group is PlanarPolygonGroup => Boolean(group));
      ...
      if (options?.select) {
        selectMeasurementById(nextEntry.id);
      }

      return {
        measurementId: nextEntry.id,
        replaced: Boolean(existingMeasurement),
      };
    },
    [coerceScriptPlanarPolygonGroup, selectMeasurementById, setScriptState]
  );

  const addScriptNodeMeasurement = useCallback(
    (params: MeasurementScriptAddNodeParams) => {
      const longitude = Number(params.longitude);
      const latitude = Number(params.latitude);
      const parsedHeightMeters = Number(params.heightMeters ?? 0);
      const heightMeters = Number.isFinite(parsedHeightMeters)
        ? parsedHeightMeters
        : 0;
      if (!Number.isFinite(longitude) || !Number.isFinite(latitude)) {
        throw new Error(
          "measurements.add(kind=node) requires finite coordinates"
        );
      }

      const id =
        params.id?.trim() ||
        `node-script-${Date.now()}-${Math.round(Math.random() * 1_000_000)}`;
      const geometryECEF = Cartesian3.fromDegrees(
        longitude,
        latitude,
        heightMeters
      );
      const labelAnchor = normalizeMeasurementLabelAnchor(params.labelAnchor);
      const labelAppearance = normalizeMeasurementLabelAppearance(
        params.labelAppearance
      );

      const existingMeasurement = scriptMeasurementsRef.current.find(
        (measurement) => measurement.id === id
      );
      const currentPointMaxIndex = scriptMeasurementsRef.current
        .filter(isPointMeasurementEntry)
        .reduce(
          (maxIndex, measurement) => Math.max(maxIndex, measurement.index ?? 0),
          0
        );
      const nextPointIndex = existingMeasurement
        ? existingMeasurement.index ?? currentPointMaxIndex
        : currentPointMaxIndex + 1;

      const nextEntry: MeasurementEntry = {
        id,
        type: CESIUM_MEASUREMENT_MODE.PointQuery,
        timestamp:
          Number.isFinite(params.timestamp) && params.timestamp
            ? params.timestamp
            : Date.now(),
        ...
        ...(params.temporary ? { temporary: true } : {}),
        pointLabelMode:
          params.pointLabelMode ?? DEFAULT_POINT_LABEL_METRIC_MODE,
      ...
      return upsertScriptMeasurementEntry(nextEntry, {
        upsertIfExists: params.upsertIfExists,
        select: params.select,
      });
    },
    [upsertScriptMeasurementEntry]
  );

  const addScriptLabelMeasurement = useCallback(
    (params: MeasurementScriptAddLabelParams) => {
      const id =
        params.id?.trim() ||
        `label-script-${Date.now()}-${Math.round(Math.random() * 1_000_000)}`;
      const anchorNodeId = params.anchorNodeId?.trim();

      let longitude = Number(params.longitude);
      let latitude = Number(params.latitude);
      let heightMeters = Number(params.heightMeters ?? 0);
      if (anchorNodeId) {
        const anchorNode = scriptMeasurementsRef.current.find(
          (measurement) =>
            isPointMeasurementEntry(measurement) &&
            measurement.id === anchorNodeId
        );
        if (!anchorNode || !isPointMeasurementEntry(anchorNode)) {
          throw new Error(
            `measurements.add(kind=label) anchorNodeId '${anchorNodeId}' was not found`
          );
        }
        longitude = anchorNode.geometryWGS84.longitude;
        latitude = anchorNode.geometryWGS84.latitude;
        heightMeters = anchorNode.geometryWGS84.height;
      }

      if (!Number.isFinite(longitude) || !Number.isFinite(latitude)) {
        throw new Error(
          "measurements.add(kind=label) requires finite coordinates or a valid anchorNodeId"
        );
      }
  ...
  const addScriptMeasurement = useCallback(
    (params: MeasurementScriptAddMeasurementParams) => {
      const payload = params.measurement;
      if (!payload || !isObjectRecord(payload)) {
        throw new Error("measurements.add requires a payload");
      }

      switch (payload.kind) {
        case "node": {
          return addScriptNodeMeasurement({
            ...(payload.payload as MeasurementScriptAddNodeParams),
            ...
            ...(params.upsertIfExists !== undefined
              ? { upsertIfExists: params.upsertIfExists }
              : {}),
          });
        }
        case "label": {
          return addScriptLabelMeasurement({
            ...(payload.payload as MeasurementScriptAddLabelParams),
            ...(params.id !== undefined ? { id: params.id } : {}),
            ...(params.name !== undefined ? { name: params.name } : {}),
            ...(params.timestamp !== undefined
              ? { timestamp: params.timestamp }
        ...
        case "measurementEntry": {
          const entryCandidate = payload.payload as MeasurementEntry;
          const nextEntry: MeasurementEntry = {
            ...entryCandidate,
          ...
          const coercedEntry = coerceScriptMeasurementEntry(nextEntry);
          if (!coercedEntry) {
            throw new Error(
              "measurements.add: invalid measurementEntry payload"
            );
          }
          return upsertScriptMeasurementEntry(coercedEntry, {
            upsertIfExists: params.upsertIfExists,
            select: params.select,
          });
        }
        case "distanceRelation": {
          const relationCandidate = {
            ...(payload.payload as PointDistanceRelation),
            ...(params.id?.trim() ? { id: params.id.trim() } : {}),
          };
          const nextRelation = coerceScriptDistanceRelation(relationCandidate);
          if (!nextRelation) {
            throw new Error(
              "measurements.add: invalid distanceRelation payload"
            );
          }

          const pointById = getPointPositionMap(scriptMeasurementsRef.current);
          if (
            !pointById.has(nextRelation.pointAId) ||
            !pointById.has(nextRelation.pointBId)
          ) {
            throw new Error(
              "measurements.add: distanceRelation references unknown point ids"
            );
          }

          const existingRelation = scriptDistanceRelationsRef.current.find(
            (relation) => relation.id === nextRelation.id
          );
          if (existingRelation && !params.upsertIfExists) {
            throw new Error(
              `distance relation id '${nextRelation.id}' already exists (set upsertIfExists=true to replace)`
            );
          }

          const nextDistanceRelations = existingRelation
            ? scriptDistanceRelationsRef.current.map((relation) =>
                relation.id === nextRelation.id ? nextRelation : relation
              )
            : [...scriptDistanceRelationsRef.current, nextRelation];
          ...
          if (params.select) {
            selectMeasurementById(nextRelation.anchorPointId);
          }

          return {
            distanceRelationId: nextRelation.id,
            replaced: Boolean(existingRelation),
          };
        }
        case "planarPolygonGroup": {
          const groupCandidate = {
            ...(payload.payload as PlanarPolygonGroup),
            ...(params.id?.trim() ? { id: params.id.trim() } : {}),
            ...(params.name?.trim() ? { name: params.name.trim() } : {}),
            ...(params.hidden !== undefined
              ? { hidden: Boolean(params.hidden) }
          ...
          const pointById = getPointPositionMap(scriptMeasurementsRef.current);
          const nextGroup = coerceScriptPlanarPolygonGroup(
            groupCandidate,
            pointById
          );
          if (!nextGroup) {
            throw new Error(
              "measurements.add: invalid planarPolygonGroup payload"
            );
          }

          const existingGroup = scriptPlanarPolygonGroupsRef.current.find(
            (group) => group.id === nextGroup.id
          );
          if (existingGroup && !params.upsertIfExists) {
            throw new Error(
              `planar polygon group id '${nextGroup.id}' already exists (set upsertIfExists=true to replace)`
            );
          }

          const nextPlanarPolygonGroups = existingGroup
            ? scriptPlanarPolygonGroupsRef.current.map((group) =>
                group.id === nextGroup.id ? nextGroup : group
              )
            : [...scriptPlanarPolygonGroupsRef.current, nextGroup];
          ...
          if (params.select) {
            selectPlanarPolygonGroupById(nextGroup.id);
          }

          return {
            planarPolygonGroupId: nextGroup.id,
            replaced: Boolean(existingGroup),
          };
        }
        default:
          throw new Error(
            `measurements.add: unsupported kind '${String(payload.kind)}'`
          );
      }
    },
    [
      addScriptLabelMeasurement,
      addScriptNodeMeasurement,
      coerceScriptDistanceRelation,
      coerceScriptMeasurementEntry,
      coerceScriptPlanarPolygonGroup,
      selectMeasurementById,
  ...
  const updateScriptMeasurement = useCallback(
    (params: MeasurementScriptUpdateMeasurementParams) => {
      const id = params.id?.trim();
      if (!id) {
        throw new Error("measurements.update requires a non-empty id");
      }
      ...
      switch (params.kind) {
        case "measurementEntry": {
          ...
          if (!existingMeasurement) {
            throw new Error(`measurement '${id}' was not found`);
          }

          const mergedEntry = {
            ...existingMeasurement,
            ...(params.patch as Partial<MeasurementEntry>),
            id: existingMeasurement.id,
          } as MeasurementEntry;
          const coercedEntry = coerceScriptMeasurementEntry(mergedEntry);
          if (!coercedEntry) {
            throw new Error(
              "measurements.update: invalid measurementEntry patch"
            );
          }

          const nextMeasurements = scriptMeasurementsRef.current.map(
            (measurement) =>
              measurement.id === id ? coercedEntry : measurement
          );
          const nextPointById = getPointPositionMap(nextMeasurements);
          const nextPlanarPolygonGroups = scriptPlanarPolygonGroupsRef.current
            .map((group) =>
              coerceScriptPlanarPolygonGroup(group, nextPointById)
            )
            .filter((group): group is PlanarPolygonGroup => Boolean(group));
        ...
        case "distanceRelation": {
          ...
          if (!existingRelation) {
            throw new Error(`distance relation '${id}' was not found`);
          }
          const mergedRelation = {
            ...existingRelation,
            ...(params.patch as Partial<PointDistanceRelation>),
            id: existingRelation.id,
          };
          const nextRelation = coerceScriptDistanceRelation(mergedRelation);
          if (!nextRelation) {
            throw new Error(
              "measurements.update: invalid distanceRelation patch"
            );
          }
          const pointById = getPointPositionMap(scriptMeasurementsRef.current);
          if (
            ...
            throw new Error(
              "measurements.update: distanceRelation references unknown point ids"
            );
          }

          const nextDistanceRelations = scriptDistanceRelationsRef.current.map(
            (relation) => (relation.id === id ? nextRelation : relation)
          );
          setScriptState(
            scriptMeasurementsRef.current,
            nextDistanceRelations,
        ...
        case "planarPolygonGroup": {
          ...
          if (!existingGroup) {
            throw new Error(`planar polygon group '${id}' was not found`);
          }
          const mergedGroup = {
            ...existingGroup,
            ...(params.patch as Partial<PlanarPolygonGroup>),
            id: existingGroup.id,
          };
          const pointById = getPointPositionMap(scriptMeasurementsRef.current);
          const nextGroup = coerceScriptPlanarPolygonGroup(
          ...
          if (!nextGroup) {
            throw new Error(
              "measurements.update: invalid planarPolygonGroup patch"
            );
          }

          const nextPlanarPolygonGroups =
            scriptPlanarPolygonGroupsRef.current.map((group) =>
              group.id === id ? nextGroup : group
            );
          setScriptState(
            scriptMeasurementsRef.current,
            scriptDistanceRelationsRef.current,
          ...
          return { planarPolygonGroupId: id };
        }
        default:
          throw new Error(
            `measurements.update: unsupported kind '${String(params.kind)}'`
          );
      }
    },
    [
  ...
  const removeScriptMeasurement = useCallback(
    (params: MeasurementScriptRemoveMeasurementParams) => {
      const id = params.id?.trim();
      if (!id) {
        throw new Error("measurements.remove requires a non-empty id");
      }

      switch (params.kind) {
        case "measurementEntry": {
          const nextMeasurements = scriptMeasurementsRef.current.filter(
            (measurement) => measurement.id !== id
          );
          if (
            nextMeasurements.length === scriptMeasurementsRef.current.length
          ) {
            throw new Error(`measurement '${id}' was not found`);
          }
          ...
          const nextPointById = getPointPositionMap(nextMeasurements);
          const nextDistanceRelations =
            scriptDistanceRelationsRef.current.filter(
              (relation) =>
                nextPointById.has(relation.pointAId) &&
                nextPointById.has(relation.pointBId)
            );
          ...
          return { measurementId: id, removed: true };
        }
        case "distanceRelation": {
          const nextDistanceRelations =
            scriptDistanceRelationsRef.current.filter(
          ...
          if (
            nextDistanceRelations.length ===
            scriptDistanceRelationsRef.current.length
          ) {
            throw new Error(`distance relation '${id}' was not found`);
          }
          setScriptState(
          ...
          return { distanceRelationId: id, removed: true };
        }
        case "planarPolygonGroup": {
          const nextPlanarPolygonGroups =
            scriptPlanarPolygonGroupsRef.current.filter(
              (group) => group.id !== id
            );
          if (
            nextPlanarPolygonGroups.length ===
            scriptPlanarPolygonGroupsRef.current.length
          ) {
            throw new Error(`planar polygon group '${id}' was not found`);
          }
          const nextDistanceRelations = scriptDistanceRelationsRef.current.map(
            (relation) =>
              relation.polygonGroupId === id
          ...
          return { planarPolygonGroupId: id, removed: true };
        }
        default:
          throw new Error(
            `measurements.remove: unsupported kind '${String(params.kind)}'`
          );
      }
    },
    [coerceScriptPlanarPolygonGroup, setScriptState]
  );

  const buildScriptRpcSuccess = useCallback(
    (
  ...
  const buildScriptRpcError = useCallback(
    (
      id: MeasurementScriptRpcId,
      code: number,
      message: string,
  ...
  const executeScriptRpcRequest = useCallback(
    (request: MeasurementScriptRpcRequest): MeasurementScriptRpcResponse => {
      const id: MeasurementScriptRpcId = request?.id ?? null;
      if (!request || (request.jsonrpc && request.jsonrpc !== "2.0")) {
        return buildScriptRpcError(
          id,
          -32600,
          "Invalid request: expected JSON-RPC 2.0 command"
        );
      }
      if (!request.method) {
        return buildScriptRpcError(
      ...
      try {
        switch (request.method) {
          case "measurements.getState":
            return buildScriptRpcSuccess(id, getScriptStateSnapshot());
          case "measurements.clearAll":
            clearAllMeasurements();
            return buildScriptRpcSuccess(id, { cleared: true });
          case "measurements.setMode": {
            const mode = request.params?.mode;
            if (!mode || !MEASUREMENT_MODE_VALUE_SET.has(mode)) {
              return buildScriptRpcError(
                id,
                -32602,
                "Invalid params: mode must be a MeasurementMode value"
              );
            }
            scriptMeasurementModeRef.current = mode;
            setMeasurementMode(mode);
            return buildScriptRpcSuccess(id, { mode });
          }
          case "measurements.select": {
            if (!request.params || !isObjectRecord(request.params)) {
              return buildScriptRpcError(
                id,
                -32602,
                "Invalid params: selection payload required"
              );
            }
            if (Array.isArray(request.params.measurementIds)) {
              selectMeasurementIds(
                request.params.measurementIds.filter(
                  (candidate): candidate is string =>
                    typeof candidate === "string"
                ),
                Boolean(request.params.additive)
              );
            } else if ("measurementId" in request.params) {
              const measurementId =
                typeof request.params.measurementId === "string"
                  ? request.params.measurementId
                  : null;
              selectMeasurementById(measurementId);
            }
            if ("planarPolygonGroupId" in request.params) {
              const planarPolygonGroupId =
                typeof request.params.planarPolygonGroupId === "string"
                  ? request.params.planarPolygonGroupId
                  : null;
              selectPlanarPolygonGroupById(planarPolygonGroupId);
            }
            return buildScriptRpcSuccess(id, { selected: true });
          }
          case "measurements.add": {
            if (!request.params || !isObjectRecord(request.params)) {
              return buildScriptRpcError(
                id,
                -32602,
                "Invalid params: add payload required"
              );
            }
            const result = addScriptMeasurement(
              request.params as MeasurementScriptAddMeasurementParams
            );
            return buildScriptRpcSuccess(id, result);
          }
          case "measurements.update": {
            if (!request.params || !isObjectRecord(request.params)) {
              return buildScriptRpcError(
                id,
                -32602,
                "Invalid params: update payload required"
              );
            }
            const result = updateScriptMeasurement(
              request.params as MeasurementScriptUpdateMeasurementParams
            );
            return buildScriptRpcSuccess(id, result);
          }
          case "measurements.remove": {
            if (!request.params || !isObjectRecord(request.params)) {
              return buildScriptRpcError(
                id,
                -32602,
                "Invalid params: remove payload required"
              );
            }
            const result = removeScriptMeasurement(
              request.params as MeasurementScriptRemoveMeasurementParams
            );
            return buildScriptRpcSuccess(id, result);
          }
          case "measurements.upsertState": {
            if (!request.params || !isObjectRecord(request.params)) {
              return buildScriptRpcError(
                id,
                -32602,
                "Invalid params: upsertState payload required"
              );
            }
            const result = applyScriptStatePatch(
              request.params as MeasurementScriptStatePatch
            );
            return buildScriptRpcSuccess(id, result);
          }
          default:
        ...
        const message =
          error instanceof Error ? error.message : "Unknown script API error";
        return buildScriptRpcError(id, -32603, message);
      }
    },
    [
      addScriptMeasurement,
      applyScriptStatePatch,
      buildScriptRpcError,
      buildScriptRpcSuccess,
      clearAllMeasurements,
      getScriptStateSnapshot,
      removeScriptMeasurement,
      selectMeasurementById,
      selectMeasurementIds,
      selectPlanarPolygonGroupById,
      setMeasurementMode,
      updateScriptMeasurement,
    ]
  );

  const executeScriptRpc = useCallback(
    (
      request: MeasurementScriptRpcRequest | MeasurementScriptRpcRequest[]
    ): MeasurementScriptRpcResponse | MeasurementScriptRpcResponse[] => {
      if (Array.isArray(request)) {
        return request.map((entry) => executeScriptRpcRequest(entry));
      }
      return executeScriptRpcRequest(request);
    },
    [executeScriptRpcRequest]
  );

  useEffect(() => {
    if (!scriptApiOptions.enabled) return;
    if (typeof window === "undefined") return;

    const namespace =
      scriptApiOptions.namespace?.trim() ||
      DEFAULT_MEASUREMENT_SCRIPT_API_NAMESPACE;
    const requestEventName =
      scriptApiOptions.requestEventName?.trim() ||
      DEFAULT_MEASUREMENT_SCRIPT_API_REQUEST_EVENT;
    const responseEventName =
      scriptApiOptions.responseEventName?.trim() ||
      DEFAULT_MEASUREMENT_SCRIPT_API_RESPONSE_EVENT;

    const win = window as unknown as Window & {
      [key: string]: MeasurementScriptWindowApi | unknown;
    };

    const apiObject: MeasurementScriptWindowApi = {
      version: "1.0.0",
      execute: executeScriptRpc,
      getState: getScriptStateSnapshot,
      requestEventName,
      responseEventName,
    };
    const previousValue = win[namespace];
    win[namespace] = apiObject;

    const handleScriptRequestEvent = (event: Event) => {
      ...
      const response = executeScriptRpc(detail.request);
    ...
    window.addEventListener(requestEventName, handleScriptRequestEvent);
    return () => {
      window.removeEventListener(requestEventName, handleScriptRequestEvent);
      if (win[namespace] === apiObject) {
        if (previousValue === undefined) {
          delete win[namespace];
        } else {
          win[namespace] = previousValue;
        }
      }
    };
  }, [
    executeScriptRpc,
    getScriptStateSnapshot,
    scriptApiOptions.enabled,
    scriptApiOptions.namespace,
    scriptApiOptions.requestEventName,
    scriptApiOptions.responseEventName,
  ]);
    ...
    (type: MeasurementListType<MeasurementMode>): MeasurementEntry[] => {
      ...
      if (type === "distanceMeasure") {
        return measurements.filter((measurement) => {
          if (!isPointMeasurementEntry(measurement)) return false;
    ...
    (payload: MeasurementCreatePayload<MeasurementEntry>): string => {
      const generatedId =
        ...
        `${payload.type}-${Date.now()}-${Math.random()
          .toString(36)
          .slice(2, 9)}`;
      const nextMeasurement: MeasurementEntry = {
        ...payload,
        id: generatedId,
  ...
  const updateMeasurementById = useCallback(
    (id: string, patch: Partial<MeasurementEntry>) => {
      if (!id) return;
      setMeasurements((prev) => {
        let hasChanged = false;
        const next = prev.map((measurement) => {
  ...
  const liveMeasurementCandidate = useMemo<MeasurementEntry | null>(() => {
