...
import { calculateUTMConvergence } from "./utm-convergence";
...
export const computeDerivedExteriorOrientation = (
  ...
  const derivedOrientation: DerivedExteriorOrientation = {
    ...
    rotation: {
      enu: {
        sourceCRS: {
          m,
          upDirectionRight: { direction: m[2], up },
        },
      },
    },
  };
  // todo normalize the orientation system from only one input
  // not mixing upDirectionRight and matrix variant of same orientation description
  ...
  const dirEnuSourceNegated = negateRow(
    derivedOrientation.rotation.enu.sourceCRS.upDirectionRight!.direction!
  );
  const upEnuSourceNegated = negateRow(
    derivedOrientation.rotation.enu.sourceCRS.upDirectionRight!.up!
  );
