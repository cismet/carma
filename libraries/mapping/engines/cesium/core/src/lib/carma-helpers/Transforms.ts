...
export const resolveLocalFrameVectors = (
  ...
  const east = normalizeDirection(cartesian3FromMetricVector3(frame.eastECEF));
  const north = normalizeDirection(
    cartesian3FromMetricVector3(frame.northECEF)
  );
  const up = normalizeDirection(cartesian3FromMetricVector3(frame.upECEF));
  if (!east || !north || !up) {
    return null;
  }

  return {
    origin: cartesian3FromMetricVector3(frame.originECEF),
    east,
    north,
    up,
  };
