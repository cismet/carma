...
import {
  ...
  startCesiumPreRenderDeltaLoop,
  type CesiumPreRenderLoopState,
} from "./scene-runtime";
...
export const createCesiumSceneOrbitController = (
  ...
  const loopState: CesiumPreRenderLoopState = {
  ...
  const cancelAnimationLoop = () => {
    loopState.removePreRenderListener?.();
    loopState.removePreRenderListener = null;
    loopState.renderRequested = false;
  };

  const scheduleAnimationLoop = () => {
    if (loopState.removePreRenderListener !== null || destroyed || !enabled) {
      return;
    }

    startCesiumPreRenderDeltaLoop(scene, loopState, {
      onStep: ({ nowMs, deltaMs }) => {
        if (!enabled) {
          visualizer?.hide();
          return CESIUM_PRE_RENDER_STOP_REASONS.CANCELED;
        }
        ...
        const timeSinceRampStartMs = nowMs - velocityRampStartTime;
        const rampProgress = Math.min(
          timeSinceRampStartMs / EASE_DURATION_MS,
          1
        );
        const easing =
