...
import { getElevationAsync } from "../terrain/Elevation";
...
export const addElevationsToGeoJson = async (
  ...
  const updatedFeatures = features.map((feature) => {
    ...
    return {
      ...feature,
      geometry: applyElevationsToGeometry(feature.geometry, elevationsByKey, {
        overrideExisting,
      }),
    };
  });
  ...
  return {
    geojson: nextGeojson,
    hasAugmentedElevation: elevationsByKey.size > 0,
  };
};
