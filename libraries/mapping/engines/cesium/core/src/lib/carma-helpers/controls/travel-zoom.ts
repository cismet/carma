...
const ensureSceneZoomLoop = (
  ...
  startCesiumPreRenderTimeline(scene, activeAnimation, {
    startedAtMs: activeAnimation.startedAtMs,
    durationMs: activeAnimation.durationMs,
    ...
    onStop: (reason) => {
      stopSceneZoomLoop(scene, activeAnimation, {
        commitFinalState: reason === CESIUM_PRE_RENDER_STOP_REASONS.COMPLETED,
      });
    },
  });
};
