...
import { loadCesiumTerrainProvider } from "../providers";
import type { TerrainProviderRecord } from "@carma/types";

/**
 * Get terrain ID - use explicit id field or extract from config
 */
const getTerrainId = (record: TerrainProviderRecord): string => {
  ...
  return config.url || "terrain-" + Math.random().toString(36).substr(2, 9);
};
 ...
 */
export const useTerrainManager = (terrainConfigs: TerrainProviderRecord[]) => {
  ...
  useEffect(() => {
    const unsubscribe = subscribe(CtxEvent.SceneReady, async () => {
      ...
      for (const terrainConfig of terrainConfigs) {
        ...
        try {
          console.debug(
            "[CESIUM|TERRAIN] Loading:",
            id,
            terrainConfig.config.url
          );

          let provider: CesiumTerrainProvider;

          if (terrainConfig.type === "ellipsoid") {
