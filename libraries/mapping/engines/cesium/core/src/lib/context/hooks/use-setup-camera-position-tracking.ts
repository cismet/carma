...
import { CtxEvent, type EmitCesiumCtxFn, type SubscribeCesiumCtxFn } from "../cesium-context-event-map";
...
export const useSetupCameraPositionTracking = (
  ...
  useEffect(function setupCameraPositionTracking() {
    ...
    const attachCameraTracking = () => {
      ...
      if (!widget || !scene?.camera) {
        console.debug("[CesiumContext] Camera tracking skipped - widget or scene not ready");
        return;
      }
