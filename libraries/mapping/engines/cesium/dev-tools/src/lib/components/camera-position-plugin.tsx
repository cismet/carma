...
type CameraPositionProps = LevaInputProps<CameraPositionValue, CameraPositionSettings, CameraPositionValue>;
...
function CameraPositionComponent() {
  ...
  const heading = camera.heading !== undefined ? ((camera.heading * (180 / Math.PI)) + 360) % 360 : 0;
  ...
  return (
    <div style={{ padding: '4px 0', fontSize: '11px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
        ...
        <div>
          <div style={{ color: '#aaa', fontSize: '10px', marginBottom: '4px', fontWeight: 500 }}>
            {showECEF ? 'ECEF' : 'WGS84'}
          </div>
          ...
          {showECEF ? (
            <>
              <div style={{ color: '#999', fontSize: '10px' }}>X: <span style={{ color: '#fff', fontFamily: 'monospace' }}>{formatLarge(cartesianPos?.x || 0)}m</span></div>
              <div style={{ color: '#999', fontSize: '10px' }}>Y: <span style={{ color: '#fff', fontFamily: 'monospace' }}>{formatLarge(cartesianPos?.y || 0)}m</span></div>
              <div style={{ color: '#999', fontSize: '10px' }}>Z: <span style={{ color: '#fff', fontFamily: 'monospace' }}>{formatLarge(cartesianPos?.z || 0)}m</span></div>
            </>
          ) : position ? (
            <>
              <div style={{ color: '#999', fontSize: '10px' }}>Lat: <span style={{ color: '#fff', fontFamily: 'monospace' }}>{(position.latitude * 180 / Math.PI).toFixed(5)}°</span></div>
              <div style={{ color: '#999', fontSize: '10px' }}>Lng: <span style={{ color: '#fff', fontFamily: 'monospace' }}>{(position.longitude * 180 / Math.PI).toFixed(5)}°</span></div>
              <div style={{ color: '#999', fontSize: '10px' }}>Alt: <span style={{ color: '#fff', fontFamily: 'monospace' }}>{formatLarge(position.height)}m</span></div>
            </>
          ) : (
        ...
        <div>
          <div style={{ color: '#aaa', fontSize: '10px', marginBottom: '4px', fontWeight: 500 }}>Orientation</div>
          <div style={{ color: '#999', fontSize: '10px' }}>H: <span style={{ color: '#fff', fontFamily: 'monospace' }}>{heading.toFixed(1)}°</span></div>
          <div style={{ color: '#999', fontSize: '10px' }}>P: <span style={{ color: '#fff', fontFamily: 'monospace' }}>{pitch.toFixed(1)}°</span></div>
          <div style={{ color: '#999', fontSize: '10px' }}>R: <span style={{ color: '#fff', fontFamily: 'monospace' }}>{roll.toFixed(1)}°</span></div>
        </div>
