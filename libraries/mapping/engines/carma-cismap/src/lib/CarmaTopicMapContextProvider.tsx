 ...
 */
const CarmaTopicMapContextInner = memo(
  ({ children }: { children: ReactNode }) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    referenceSystemDefinitionRef.current = (
      reactCismapContext as any
    ).referenceSystemDefinition;

    const getRoutedMapRef = useCallback(() => routedMapRefRef.current, []);
    const getReferenceSystem = useCallback(
      () => referenceSystemRef.current,
      []
    );
    const getReferenceSystemDefinition = useCallback(
      () => referenceSystemDefinitionRef.current,
      []
    );
...
export const CarmaTopicMapContextProvider = memo(
  ({ children, config }: CarmaTopicMapContextProviderProps) => {
