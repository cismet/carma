...
export const cleanupLegacyProjectedTerrainCache = (): Promise<boolean> =>
  new Promise((resolve) => {
    ...
    let releaseLock = () => {};
    const finished = new Promise<void>((release) => {
      releaseLock = release;
    });
    ...
    const timer = setTimeout(
      () => finish(false),
      LEGACY_PROJECTED_TERRAIN_CACHE.timeoutMs
    );
    const start = async () => {
      ...
      if (
        !existing.some(
          (entry) => entry.name === LEGACY_PROJECTED_TERRAIN_CACHE.databaseName
        )
      ) {
        finish(false);
        return;
        ...
        try {
          if (
            !database.objectStoreNames.contains(
              LEGACY_PROJECTED_TERRAIN_CACHE.storeName
            )
          ) {
            finish(false);
            return;
          ...
          transaction
            .objectStore(LEGACY_PROJECTED_TERRAIN_CACHE.storeName)
            .clear().onerror = () => finish(false);
        } catch {
          finish(false);
        }
      };
    ...
    try {
      ...
      if (typeof locks?.request === "function") {
        void locks
          .request(
            LEGACY_PROJECTED_TERRAIN_CACHE.lockName,
            {
              mode: "exclusive",
              ifAvailable: true,
            },
