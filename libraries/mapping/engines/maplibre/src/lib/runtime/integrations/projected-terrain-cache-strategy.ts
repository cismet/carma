...
export const createProjectedTerrainCacheStrategy = (
  ...
  const profiles = manager.register(
    "terrain-cache-strategies",
    PROFILE_VERSION
  );
  ...
  const profileKey = (bytes: number) =>
    `${environment()}|${version}|${sizeClass(bytes)}`;
  const readProfile = async (
    ...
    const value = (
      await (touch
        ? profiles.get<Profile>(key)
        : profiles.get<Profile>(key, { touch: false }))
    )?.value;
  ...
  const decode = async (
    ...
    if (
      ...
      return null;
    try {
      const unpacked = await decodeTypedBinaryRecord(stored.payload, {
  ...
  return {
    ...
    canWrite(key: string) {
      return !disposed && !provenSlowKeys.has(key);
    },
    ...
    async inspectProfiles() {
      return {
        ...
        pendingSeed:
          pendingSeed?.owner === owner && pendingSeed.expiresAt > Date.now()
            ? {
                bytes: pendingSeed.row.bytes,
                observedReuseCount: pendingSeed.row.hits,
                expiresAt: pendingSeed.expiresAt,
                prepareMs: pendingSeed.prepareMs,
              }
            : null,
      };
    },
    async updateCosts(key: string, restoreMs: number) {
      ...
      if (!disposed && Number.isFinite(restoreMs) && restoreMs > 0) {
        try {
          ...
          const row = (await records.inspect())?.find(
          ...
          if (
            ...
            Number.isFinite(row.recomputeMs) &&
            row.recomputeMs >= 0 &&
            !isDerivedCacheSavingSufficient(
              { ...row, restoreMs },
            ...
            provenSlowKeys.delete(key);
            provenSlowKeys.add(key);
            if (provenSlowKeys.size > 256)
              provenSlowKeys.delete(provenSlowKeys.values().next().value!);
            if (
              !pendingSeed &&
              !capturingSeed &&
              typeof navigator !== "undefined" &&
              ...
              try {
                ...
                const bytes = entry
                  ? Math.max(row.bytes, entryBufferBytes(entry))
                  : 0;
                if (
                  !disposed &&
                  entry &&
          ...
          /* Optional calibration must not prevent normal rejection. */
        }
      }
    ...
    async encode(entry: CachedProjectedTerrainTile, bytes: number) {
      try {
        ...
        return {
          payload,
          bytes: "payload" in payload ? payload.payload.size : bytes,
        };
      } catch {
      ...
      try {
        return await navigator.locks.request(
          ...
          async (lock) => {
            ...
            const candidates = [
              ...(seed ? [seed.row] : []),
              ...(rows ?? []).filter((row) => row.key !== seed?.row.key),
            ].filter(
              (row) =>
                (row.hits ?? 0) >= 1 &&
                row.bytes <= CACHE_MAX_BYTES &&
                row.recomputeMs !== undefined &&
                row.restoreMs !== undefined
            );
            for (const row of candidates) {
              ...
              const stored = fromSeed
                ? null
                : await records.get<unknown>(row.key, { touch: false });
              const entry = fromSeed
                ? seed!.entry
                : stored
                ? await decode(stored.value)
                : null;
              if (!entry) continue;
              ...
              try {
                ...
                for (const format of ["native", "binary", "meshopt"] as const) {
                  ...
                  if (signal?.aborted) return false;
                  const bytes =
                    "payload" in payload ? payload.payload.size : row.bytes;
                  const key = crypto.randomUUID();
                  keys.push(key);
                  // Unknown trials only use spare space; never evict valuable data.
                  if (!(await probes.put(key, payload, { bytes })))
                    return false;
                  const prepareMs =
                    performance.now() -
                    start +
                    (fromSeed ? seed!.prepareMs : 0);
  ...
  if (a && typeof a === "object") {
    ...
    return (
      ...
      left.every(
        ([key, value]) =>
          Object.hasOwn(b, key) &&
          equalRecord(value, (b as Record<string, unknown>)[key])
      )
    );
  }
