...
/**
 * Build a ViewState from angle-based orbit parameters.
 *
 ...
 */
export const buildViewState = (input: AngleBasedViewInput): ViewState => {
  ...
  const { east, north, up } = anchoredOrbitToEnuOffset({
...
/**
 * Build a ViewState directly from ECEF positions plus a quaternion that
 * is already expressed in the shared local Y-up scene basis.
 * Used by 3D adapters after projecting engine-native world orientation into
 * the shared anchor-local convention.
 */
export const buildViewStateFromEcef = (input: {
  anchor: Vector3;
