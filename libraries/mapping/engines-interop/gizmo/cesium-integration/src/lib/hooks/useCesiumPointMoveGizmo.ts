...
import {
  ...
  type ScreenPoint2,
} from "@carma-mapping/engines-interop/gizmo/cesium-core";
...
export const useCesiumPointMoveGizmo = (
  ...
  const discVisualizerRef = useRef<DiscVisualizer | null>(null);
  ...
  const getGroundPointWithoutGizmoVisuals = useCallback(
    (clientX: number, clientY: number): Cartesian3 | null => {
      ...
      if (discVisualizer?.isVisible) {
        discVisualizer.hide();
        hiddenVisualizers.push(discVisualizer);
      }
  ...
  useEffect(() => {
    ...
    if (showDiscVisualizer) {
      const disc = createDiscVisualizer(`point-move-disc-${movePoint.id}`, {
        origin: movePoint.geometryECEF,
        upVector: initialAxisDirection,
        radius,
        screenPixelRadius: DISC_SCREEN_PIXEL_RADIUS,
        color: DISC_FILL_COLOR,
        unitCircleSegments: 24,
      });
      disc.attach(scene, () => scene.requestRender());
      ...
      updatePosition: (elementDiv) => {
        try {
          ...
          if (discInteractionPath) {
            if (showDiscVisualizer && activeOutline) {
          ...
          if (rotationHandle) {
            if (showRotationHandle && showDiscVisualizer && activeOutline) {
