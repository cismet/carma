...
export function InfoBoxMeasurement3D({ pixelWidth = 350 }) {
  ...
  const pointMeasurementById = useMemo(
    () =>
      new Map(
        pointMeasurements.map((measurement) => [measurement.id, measurement])
      ),
    [pointMeasurements]
  );
  ...
  const navigationEntries = useMemo<MeasurementNavigationEntry[]>(() => {
    const standaloneMeasurementEntries: MeasurementNavigationEntry[] =
      measurements
        .filter((measurement) => {
        ...
        .map((measurement) => ({
          ...
          id: measurement.id,
          measurement,
          sortTimestamp: measurement.timestamp ?? 0,
        }));
    const polylineEntries: MeasurementNavigationEntry[] = planarPolygonGroups
      .filter((group) => !group.closed && group.vertexPointIds.length > 0)
      .map((group) => {
        const sortTimestamp = group.vertexPointIds.reduce(
          (maxTimestamp, pointId) => {
            const pointTimestamp =
              pointMeasurementById.get(pointId)?.timestamp ?? 0;
            return Math.max(maxTimestamp, pointTimestamp);
          },
          0
        );
        return {
          kind: "polyline",
          id: `polyline:${group.id}`,
          polylineGroupId: group.id,
          sortTimestamp,
        };
      });
    return [...standaloneMeasurementEntries, ...polylineEntries].sort(
      (left, right) =>
  ...
  const currentMeasurement =
    currentNavigationEntry?.kind === "measurement"
      ? currentNavigationEntry.measurement
      : undefined;
  ...
  const contextFocusedPlanarPolygonGroupId =
    selectedPlanarPolygonGroupId ?? activePlanarPolygonGroupId;
  const focusedPlanarPolygonGroupId =
    contextFocusedPlanarPolygonGroupId ??
    (currentNavigationEntry?.kind === "polyline"
      ? currentNavigationEntry.polylineGroupId
      : null);
  ...
  const selectedPolylineSegmentLineMode: PolylineSegmentLineMode =
    useMemo(() => {
      if (!selectedPlanarPolygonGroup) return "components";
      if (selectedPlanarPolygonGroup.closed) return "direct";
      return selectedPlanarPolygonGroup.segmentLineMode ?? "components";
    }, [selectedPlanarPolygonGroup]);
  const suppressPolygonAreaInDirectPolylineMode =
    Boolean(selectedPolylineSummary) &&
    selectedPolylineSegmentLineMode === "direct";
  const updateSelectedPolylineSegmentLineMode = (
    nextMode: PolylineSegmentLineMode
  ) => {
    if (!selectedPlanarPolygonGroup || selectedPlanarPolygonGroup.closed)
      return;
    ...
    if (selectedMeasurementId) {
      selectedIndex = navigationEntries.findIndex(
        (entry) =>
          ...
          entry.measurement.id === selectedMeasurementId
      );
    }
    if (selectedIndex < 0 && focusedPlanarPolygonGroupId) {
      selectedIndex = navigationEntries.findIndex(
        (entry) =>
          entry.kind === "polyline" &&
          entry.polylineGroupId === focusedPlanarPolygonGroupId
      );
    }
  ...
  useEffect(() => {
    if (!currentNavigationEntry || currentNavigationEntry.kind !== "polyline") {
      return;
    }
    if (
      contextFocusedPlanarPolygonGroupId ===
    ...
    selectPlanarPolygonGroupById(currentNavigationEntry.polylineGroupId);
  }, [
    contextFocusedPlanarPolygonGroupId,
  ...
  const selectNavigationEntry = (
    ...
    if (entry.kind === "polyline") {
      selectMeasurementById(null);
      selectPlanarPolygonGroupById(entry.polylineGroupId);
      return;
    }
    selectPlanarPolygonGroupById(null);
  ...
  const selectedPolygonHorizontalAreaSquareMeters = useMemo(() => {
    if (suppressPolygonAreaInDirectPolylineMode) return 0;
    if (selectedConnectedPlanarPolygonGroups.length === 0) return 0;
    const pointById = new Map(
      pointMeasurements.map((point) => [point.id, point])
    );
  ...
  const showHorizontalAreaForType =
    ...
    selectedPolygonSurfaceTypeValue === "footprint";
  const selectedPolygonPrimaryAreaSquareMeters =
    suppressPolygonAreaInDirectPolylineMode
      ? 0
    ...
    navigationEntries.length > 0 ? (
      <>
        <div className="flex justify-center items-center w-full px-2 mt-1 pt-1">
          ...
          >
            {navigationEntries.length} Messungen verfügbar
          </span>
        </div>
        <div className="flex justify-between items-center w-full px-2 mt-0 mb-1">
          ...
          <span className="mx-4">
            {currentIndex + 1} von {navigationEntries.length}
          </span>
          <a
            className="renderAsLink text-[#0078a8] cursor-pointer"
              ...
              <div className="mt-1 mb-0 w-full px-2">
                ...
                <div className="w-full text-[10px] font-normal text-gray-500 -mt-1 min-h-[16px] flex items-center gap-2 whitespace-nowrap">
                  {selectedPolylineSummary ? (
                    <span>
                      Polygonzug •{" "}
                      {formatNumber(selectedPolylineSummary.totalLengthMeters)}{" "}
                      m • Höhendifferenz:{" "}
                      {formatNumber(
                        Math.abs(
                ...
                <div className="flex justify-between items-start gap-2">
                  ...
                  >
                    <MeasurementTitle
                      ...
                      order={navigationEntries.length - currentIndex}
                      title={
                        getCustomPointMeasurementName(
                      ...
                      placeholderText={`${DEFAULT_POINT_MEASUREMENT_PLACEHOLDER} #${
                        navigationEntries.length - currentIndex
                      }`}
                      clearPlaceholderOnFocus
                      showOrder={false}
                      collapsedContent={
                    ...
                    <>
                      ...
                      >
                        ...
                        <Switch
                          size="small"
                          checked={
                            selectedPolylineSegmentLineMode === "components"
                          }
                          onChange={(checked) =>
                      ...
                      <div className="mb-1 flex flex-wrap items-baseline gap-x-2 gap-y-1 text-[10px]">
                        ...
                        <span className="tabular-nums">
                          {formatNumber(selectedPolylineSummary.descentMeters)}{" "}
                          m
                        </span>
