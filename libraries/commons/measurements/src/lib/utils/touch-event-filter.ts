...
export function createTouchEventFilter(): TouchEventFilter {
  ...
  return {
    ...
    markValidTouch(touchId: number): void {
      ...
      console.log('[TouchEventFilter] Valid touchstart, tracking touch ID:', touchId);
    },
    
    markInvalidTouch(touchId?: number): void {
      lastInvalidTouchTime = Date.now();
      console.warn('[TouchEventFilter] Invalid touchstart detected (NaN coords), marking for skip');
    ...
    get lastInvalidTouchTime() { return lastInvalidTouchTime; }
  };
}
