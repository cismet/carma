...
def write_generated_typescript(output_directory: Path, provenance, tiles):
    loader_lines = [
        f'  "{tile["id"]}": () => import("./gcg2016/{tile["id"]}.json").then((module) => module.default),'
        for tile in tiles
    ]
...
def main():
    ...
    provenance = {
        ...
        "source": {
            ...
            "adaptation": "source Float32 samples unchanged; spatially subset and repackaged as independently loadable CARMA JSON tiles",
    ...
    for tile in tiles:
        (arguments.output_directory / f"{tile['id']}.json").write_text(
