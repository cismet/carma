...
export interface PanoramaViewerProps {
  ...
  initialYaw?: number;
  /**
   * Optional Escape handler. Standalone use (PanoramaLightBox) passes its close
   * fn; when embedded in the media lightbox the shell owns Escape, so this is
   * omitted to avoid a double close.
   */
  onEscape?: () => void;
}
...
/**
 * Embeddable 360° panorama viewer (pannellum). Fills its parent (100% × 100%)
 * and renders no fullscreen chrome of its own — the host (PanoramaLightBox or
 * the media lightbox shell) provides the overlay, title and close button. It
 * carries the in-viewer interaction layer: the Street-View cursor arrow, the
 * navigation hotspots, and the bottom-centre "go back" zone.
 */
export const PanoramaViewer = ({
  ...
  return (
    <div
      className="carma-panorama-viewer"
