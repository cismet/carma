...
const InfoPanel: React.FC<InfoPanelProps> = ({ data, onClose }) => {
  const { activeMeasurementPoints, enableMeasurement, viewer } =
    useCesiumMeasurements();

  if (!data && !enableMeasurement) return null;

  const isMeasurementPanelActive =
    enableMeasurement && activeMeasurementPoints.length > 0;

  return (
    <div className="panel-base info-panel">
      <div className="info-panel-header">
        <Title level={5} className="info-panel-title">
          {isMeasurementPanelActive ? "Aktive Messung" : "Punktabfrage"}
        </Title>
        <Button
          type="text"
          size="small"
      ...
      {/* Display point query data if available AND measurement is NOT active */}
      {!isMeasurementPanelActive && data && (
        <>
