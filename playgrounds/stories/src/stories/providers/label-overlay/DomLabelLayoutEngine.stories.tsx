...
const resolvePillCapCenterPoint = (
  ...
  connectorPoint: ScreenPoint
): ScreenPoint => {
  if (attach === "left") {
    return { x: connectorPoint.x + LABEL_CAP_RADIUS_PX, y: connectorPoint.y };
  }
  if (attach === "right") {
    return { x: connectorPoint.x - LABEL_CAP_RADIUS_PX, y: connectorPoint.y };
  }
...
const DomLabelLayoutEngineStory = ({
  showGrid,
  gridExtent,
  gridStep,
  labelCount,
  ...
  const statusValues = useMemo(
    () => [
      `${forceEnabled ? "forced" : "static"} layout`,
      ...
      >
        ...
        <div style={{ position: "absolute", inset: 0, pointerEvents: "none" }}>
          {anchorPoints.map((pointData) => {
            ...
            const stemEndPoint = resolvePillCapCenterPoint(
              placement.attach,
              connectorPoint
            );
...
const meta: Meta<DomLabelLayoutStoryArgs> = {
...
export const DomLayoutEngine: StoryObj<DomLabelLayoutStoryArgs> = {
