...
const CANVAS_HEIGHT_PX = RATE_CANVAS_HEIGHT_PX + COUNT_CANVAS_HEIGHT_PX;
const CANVAS_BACKGROUND = "#f8fafc";
const TRACE_LINE_WIDTH_PX = 1;
const SCROLL_RATE_PX_PER_S = 144;
...
const formatMetricValue = (metric: CursorRateDiagnosticsMetricState) => {
  ...
  if (metric.kind === "ratio") {
    ...
    const minPercent =
      metric.minValue !== null ? Math.round(metric.minValue * 100) : "--";
    const maxPercent =
      metric.maxValue !== null ? Math.round(metric.maxValue * 100) : "--";
    return `${currentPercent}% [${minPercent}-${maxPercent}]`;
  }

  const currentHz = Math.round(metric.latestValue);
  const minHz = metric.minValue !== null ? Math.round(metric.minValue) : "--";
  const maxHz = metric.maxValue !== null ? Math.round(metric.maxValue) : "--";
  return `${currentHz} Hz [${minHz}-${maxHz}]`;
};

const getMetricScaleMax = (
  metric: CursorRateDiagnosticsMetricState,
  maxRateHz: number
) => (metric.kind === "ratio" ? 1 : Math.max(maxRateHz, 1));
...
export const createCursorRateDiagnosticsController = ({
  ...
  const resizeCanvas = () => {
    ...
    const rateHeight = Math.max(
      1,
      Math.round(RATE_CANVAS_HEIGHT_PX * devicePixelRatio)
    );
  ...
  const buildRowLabels = () => {
    ...
    const visibleRateMetrics = getVisibleMetrics();
    const rateRowHeight = RATE_CANVAS_HEIGHT_PX / Math.max(visibleRateMetrics.length, 1);
    ...
    countMetrics.forEach((metric, rowIndex) => {
      ...
      row.style.cssText = `${ROW_LABEL_STYLE};top:${
        RATE_CANVAS_HEIGHT_PX + countRowHeight * (rowIndex + 0.5)
      }px`;
      const nameSpan = document.createElement("span");
      nameSpan.style.cssText = "color:#020617";
      nameSpan.textContent = metric.label + "  ";
  ...
  const updateLabelValues = () => {
    ...
    countMetrics.forEach((metric) => {
      ...
      el.textContent = String(Math.round(metric.latestValue));
    });
  };

  const drawNextRateColumn = () => {
    if (!rateContext || rateCanvas.width <= 1) {
      return;
    }
    ...
    visibleMetrics.forEach((metric, rowIndex) => {
      ...
      const safeMetricMax = getMetricScaleMax(metric, currentOptions.maxRateHz);
  ...
  const applySupportState = () => {
    ...
    syncStatusValue(
      maxRateElement,
      `max ${currentOptions.maxRateHz} Hz current ${currentPollRateHz.toFixed(1)} Hz`
    );
    buildRowLabels();
  };
  ...
  const pushMetricValue = (
    ...
    metric.lastEventTimeMs = performance.now();
  ...
  const tickAnimationFrame = (timestampMs: number) => {
    ...
    if (lastRafTimestampMs >= 0) {
      ...
      const msPerPx = 1000 / SCROLL_RATE_PX_PER_S;
