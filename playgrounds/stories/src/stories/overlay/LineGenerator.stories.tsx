...
const splitRepresentativeSectionsIntoColumns = (
  sections: readonly RepresentativeSection[]
): [RepresentativeSection[], RepresentativeSection[]] => {
  const columns: [RepresentativeSection[], RepresentativeSection[]] = [[], []];
  const heights = [0, 0];

  sections.forEach((section) => {
    const estimatedHeight = section.rows.length * 22 + 30;
...
const RepresentativeCasesStory = ({
  ...
  const [leftColumn, rightColumn] = useMemo(
    () => splitRepresentativeSectionsIntoColumns(sections),
    [sections]
  );

  return (
    <div style={{ ...matrixFrameBaseStyle, overflow: "visible" }}>
      <div
        style={{
          maxWidth: 1120,
          margin: "0 auto",
      ...
      >
        ...
        >
          {`Representative Cases (${capStyle} caps)`}
        </div>
        <div
          style={{
            display: "flex",
        ...
        >
          {[leftColumn, rightColumn].map((column, columnIndex) => (
            ...
            >
              {column.map((section) => (
                <section key={section.id} style={{ marginBottom: 10 }}>
                  <div
                    style={{
                      color: "#000",
                      fontSize: 12,
