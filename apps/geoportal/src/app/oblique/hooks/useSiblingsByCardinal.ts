...
function findSameLineNeighbors(
  ...
  // Exact ±1 by waypointIndex, same line and sector, no distance check
  ...
  if (forward && backward)
    return { forward, backward, forwardExact, backwardExact };
  // Fallback: nearest within threshold in correct direction(s)
...
> {
  ...
  const siblingsByCardinal = useMemo(() => {
    ...
    if (!current || !imageRecords || imageRecords.size === 0) return map;
