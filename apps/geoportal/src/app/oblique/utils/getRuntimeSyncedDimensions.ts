...
const readRuntimeSyncedSizeFromViewport = (
...
const getRuntimeSyncedSize = (
  ...
  if (typeof syncedSize !== "number") {
    console.debug("getRuntimeSyncedSize: unsupported or missing frustum; skip");
...
export const getSceneSyncedDimensions = (
  ...
  const baseSize = Number(
    readRuntimeSyncedSizeFromViewport(
...
export const getRuntimeSyncedDimensions = (
  ctx: ReturnType<typeof useCesiumContext>,
  isVertical: boolean,
  imageAspectRatio: number,
  baseScaleFactor: number,
  ...
  const baseSize = Number(getRuntimeSyncedSize(ctx, overrideFov) ?? 1);
  const widthScaleFactor =
    baseScaleFactor * (isVertical ? imageAspectRatio : 1);
  const heightScaleFactor =
    baseScaleFactor * (isVertical ? 1 : 1 / imageAspectRatio);
