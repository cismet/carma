 ...
 * and that mode adds its row again at startup, with one exception: a mode row
 * that carries `tools` holds its own relaunch config (the time series embeds
 * its series there, the way a workflow card does) and is exactly what its mode
 * needs at boot.
 *
