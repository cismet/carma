...
export const useCameraOrbit = ({
  ...
  const stopOrbit = useCallback((options?: StopOrbitOptions) => {
  ...
  const ensureVisualizer = useCallback(() => {
    if (!scene || !isOrbiting || visualizerRef.current || stopPendingRef.current) {
  ...
  useEffect(() => {
    ...
    const startPointer = (movement: { position?: { x: number; y: number } }) => {
      isPointerDownRef.current = true;
      pointerDownPositionRef.current = getEventPosition(movement);
    };

    const handlePointerMove = (movement: { endPosition?: { x: number; y: number } }) => {
    ...
    const handleZoom = () => {
      ...
      if (isOrbiting) {
        ...
        if (pickResult?.scenePosition) {
          ...
          visualizer.show();
          visualizer.update(pickResult.scenePosition, surfaceNormal, scene.camera.position);
        }
  ...
  }, [ensureVisualizer, scene, angularVelocity, isOrbiting, restartDelayMs, startVelocityRamp]);
    ...
    const animate = (currentTime: number) => {
      ...
      if (isDraggingRef.current) {
        ...
        if (pickResult?.scenePosition) {
          ...
          dragVisualizer?.show();
          dragVisualizer?.update(pickResult.scenePosition, surfaceNormal, camera.position);
        }
      ...
      const isAccelerating = targetVelocityRef.current >= rampStartVelocityRef.current;
      ...
      currentVelocityRef.current =
        ...
        (targetVelocityRef.current - rampStartVelocityRef.current) * easedProgress;
  ...
  }, [ensureVisualizer, scene, enabled, isOrbiting, isStopping, angularVelocity, startVelocityRamp]);
