...
2. Der Vektor-Layer zeigt die ALKIS-Gebäude (`tiles.cismet.de/alkis`, Source-Layer `building`) und die erfassten Leerstände (GeoJSON-Source, per `setData` befüllt). Ein Tipp auf ein Gebäude selektiert es und zeigt die ALKIS-Infobox des Geoportals (Kopfzeile Gebäudetyp, Adresse, Funktion, Grundfläche) mit drei Symbolen: Zoom auf das Gebäude, Info-Symbol für das ALKIS-Datenblatt (`AlkisSIM` aus der Wuppertal-Collab) und Plus-Symbol für die Erfassung. Zum eigenen Standort geht es mit dem Locate-Control.
